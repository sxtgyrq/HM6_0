...
";
        ...
        public static bool Trade(ref long stockScorePrice)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(Connection.ConnectionStr))
            {
                ...
                {
                    //如error001.jpg所示
                    Console.WriteLine($"DalOfAddress.StockBuy.Trade Error{e.Message}");
                    File.WriteAllText($"{DateTime.Now.ToString("erroryyyyMMddHHmmss.txt")}", $"DalOfAddress.StockBuy.Trade Error{e.Message}");
                    return false;
                }
                using (MySqlTransaction tran = con.BeginTransaction())
                {
