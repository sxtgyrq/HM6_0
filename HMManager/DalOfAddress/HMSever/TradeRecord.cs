                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    ...
                    {// raceRecordIndex READ, introducedetai READ,traderewardtimerecord READ,administratorwallet READ,
                     ...
                     // LockTabel(tran, con);
                        ...
                        {
                            addrBussiness = lockBusinessAddr(addrBussiness, tran, con);
                            int tradeIndexInDB;
                        ...
                        }
                        {
                            ...
                            if (DalOfAddress.HMSever.MoneyAdd.GetMoney(con, tran, addrFrom) > TradeStockCost)
                            {
                                long subsidizeGet, subsidizeLeft;
                                DalOfAddress.HMSever.MoneyGet.GetSubsidizeAndLeft(con, tran, addrFrom, TradeStockCost, out subsidizeGet, out subsidizeLeft);
                                if (subsidizeLeft > 0)
                                {
                                    string sQL = mysql;
                                    // long moneycount;
                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        }
                        {
                            ...
                            var moneyNow = DalOfAddress.HMSever.MoneyAdd.GetMoney(con, tran, addrFrom);
                            if (moneyNow > costMoney)
                            {
                                long subsidizeGet, subsidizeLeft;
                                DalOfAddress.HMSever.MoneyGet.GetSubsidizeAndLeft(con, tran, addrFrom, costMoney, out subsidizeGet, out subsidizeLeft);
                                if (subsidizeLeft > 0)
                                {
                                    string sQL = mysql;
                                    int row;
            ...
            using (MySqlConnection con = new MySqlConnection(Connection.ConnectionStr))
            {
                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            // const long costMoney = 1000000;
                            if (DalOfAddress.HMSever.MoneyAdd.GetMoney(con, tran, addrFrom) > passObj.Cost)
                            {
                                long subsidizeGet, subsidizeLeft;
                                DalOfAddress.HMSever.MoneyGet.GetSubsidizeAndLeft(con, tran, addrFrom, passObj.Cost, out subsidizeGet, out subsidizeLeft);
                                if (subsidizeLeft > 0)
                                {
                                    string sQL = mysql;
                                    // long moneycount;
...
总的来说，读取锁定允许其他事务读取数据，但不允许写入，而写入锁定则阻止其他事务对表进行任何操作。在实际应用中，根据需要选择合适的锁定级别来保护数据的完整性和一致性，同时提高并发性能。
            ...
            var tr = DalOfAddress.HMSever.TradeReward.GetByStartDate(int.Parse(agp.Time));
            if (tr == null)
            {
                    ...
                    else
                    {
                        ...
                        if (countOfRewardByRaceTime + countOfRewardByShareCount == agp.Msgs.Count)
                        {
                            ...
                            for (int i = 0; i < countOfRewardByRaceTime; i++)
                            {
                                ...
                                businessAddr = lockBusinessAddr(businessAddr, tran, con);
                                if (formatIsWrite)
                                {
                                    int tradeIndexInDB;
                                    {
        ...
        private static string lockBusinessAddr(string businessAddr, MySqlTransaction tran, MySqlConnection con)
        {
            string btcAdd;
            var sQL = $"SELECT btcAdd FROM administratorwallet WHERE btcAdd ='{businessAddr.Trim()}' FOR UPDATE";
            using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
            {
                ...
                using (MySqlTransaction tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        }
                        {
                            ...
                            if (DalOfAddress.HMSever.MoneyAdd.GetMoney(con, tran, addrTo) > costScore + costMoneyOfBuyer)//这里要扣积分卖家（股份买家）的积分。
                            {
                                long subsidizeGet, subsidizeLeft;
                                DalOfAddress.HMSever.MoneyGet.GetSubsidizeAndLeft(con, tran, addrTo, costScore + costMoneyOfBuyer, out subsidizeGet, out subsidizeLeft);
                                if (subsidizeLeft > 0)//这里千万不能等于0
                                {
                                    DalOfAddress.HMSever.MoneyAdd.AddMoney(addrFrom, costScore);
                                    string sQL = mysql;
                                    // long moneycount;
                                    using (MySqlCommand command = new MySqlCommand(sQL, con, tran))
                                    {
