...
私钥丢失，无法找回！`,
    ...
    updateMsgNeedToSignTextareaAtStocktradecenter: function () {
        ...
        switch (stocktradecenter.state) {
            ...
            case 'zhifubao':
                {
                    var msg = '北京时间' + stocktradecenter.data.DateTimeStr + ',' + stocktradecenter.data.BTCAddr + `用${(stocktradecenter.data.RewardScoreCost/100).toFixed(2)}积分与${stocktradecenter.data.RewardSotoshiCost}聪股点换取支付宝红包。nyrq123.com`;
                    //msgNeedToSignTextareaAtStocktradecenter
                    if (document.getElementById('msgNeedToSignTextareaAtStocktradecenter') == null) { }
                    else {
                        document.getElementById('msgNeedToSignTextareaAtStocktradecenter').value = msg;
    ...
    formatIsRight: function (inputV) {
        ...
        const regex4 = /^北京时间\d{4}年\d{2}月\d{2}日\d{2}时\d{2}分,[13]{1}[1-9A-HJ-NP-Za-km-z]{25,34}用(?!0(?!\.))\d{1,}\.\d{2}积分与(?!0(?!\.))\d{1,}聪股点换取支付宝红包。nyrq123\.com$/;
